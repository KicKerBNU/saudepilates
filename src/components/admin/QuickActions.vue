<template>
  <div class="mt-8 px-4 sm:px-0">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <router-link :to="{name: 'ProfessorPayments'}" class="bg-white overflow-hidden shadow rounded-lg hover:bg-gray-50">
        <div class="px-4 py-5 sm:p-6">
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <h3 class="text-lg font-medium text-gray-900">{{ $t('admin.professorPaymentsTitle') }}</h3>
              <p class="mt-1 text-sm text-gray-500">
                {{ $t('admin.professorPaymentsDesc') }}
              </p>
            </div>
          </div>
        </div>
